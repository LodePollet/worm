...
lattice = sys.argv[1]
with open(f"./ED_canonical/{lattice}.json",'r') as edjsonf:
...
print("Green's function at zero momentum")
try: #might not exist in edOut (json file)
        gft_ed = np.array(edOut['gft'], dtype=float)
        gft = np.array(h5res['Greenfun_p0_tau']['mean']['value'])
        gfterr = np.array(h5res['Greenfun_p0_tau']['mean']['error'])
        # Only look at positive times (second half of gft)
        gft = gft[gft.size//2:]
        gfterr = gfterr[gfterr.size//2:]
        if veccheck(gft_ed, gft, gfterr, tolfac=1):
                print("success")
        else:
                print(gft_ed)
                print(gft, gfterr)
...
nprtcls = float(edOut['Np'])
mu = h5Out['parameters']['mu'][()]
...
Epot_ed = float(edOut['Epot'])
Epot = np.array(h5res['Potential_Energy']['mean']['value']) + mu * nprtcls
...
Etot = np.array(h5res['Total_Energy']['mean']['value'])  + mu * nprtcls
